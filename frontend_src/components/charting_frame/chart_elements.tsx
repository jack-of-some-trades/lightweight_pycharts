...
function ScaleToggle(props:scale_props){
    ...
    props.scale_ref.addEventListener('mouseenter', ()=>{
        ...
        const pos = rect.width/2 - 14
        setWrapperStyle(props.right? {right:`${pos}px`} : {left:`${pos}px`})
        setShow(true)
    },{signal:event_cleaner.signal})

    //Set to Mode if not already, otherwise reset to normal
    const setMode = (mode:number) => props.scale_signal[1](props.scale_signal[0]() !== mode? mode : 0)

    // percent and indexd to 100 aren't as common. Commented out for now since they clutter the UI.
    return <Show when={show()}>
        <div ref={divRef} class={props.right?'scale_buttons_right':'scale_buttons_left'} style={wrapperStyle()}>
            <TextIcon 
                text={"L"}
                onClick={()=>setMode(1)}
