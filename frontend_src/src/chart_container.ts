 ...
 */
export class chart_container extends container{
    layout: Container_Layouts | null
    frames: frame[] = []
    flex_divs: flex_div[] = []

    constructor(super_args:container_args) {
        super(super_args)
        this.div.classList.add('layout_container_row')
        this.layout = null
        this.resize()
    }
    ...
    private _create_frame(specs: flex_div, id: string = ''): frame {
        let new_frame = new frame(id, specs.div, this.update_tab)
